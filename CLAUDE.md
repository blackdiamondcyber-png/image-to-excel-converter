...
- **`NEXT_PUBLIC_*` vars are inlined at build time** — if you add/change them on Vercel, you must trigger a redeploy for the client bundle to pick up the new values. Server-only vars (`ANTHROPIC_API_KEY`, `FIREBASE_*`) do NOT change a running deployment either: a deployment keeps the env it was built with, and an API or dashboard redeploy of an existing deployment inherits that snapshot. A changed value needs a fresh build (a push, or `create_deployment` from the git ref).
...
- **Service worker** (`/public/sw.js`) uses network-first for navigation with offline fallback, cache-first for static assets, and never caches API routes or Firebase calls. Cache version is `"rohan-v4"` — bump when changing caching strategy.
